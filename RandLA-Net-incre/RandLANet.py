...
def fuse_feat_cat(fpn_feat: dict, pts_3d, pc_feat, calib):
    ...
    for i in range(BS):
        ...
        cur_sampled_feat = cur_sampled_feat.permute(1, 0, 2).contiguous().view(num_pts,
                                                                               -1)  # (20, num_pts, 512) to (num_pts, 20, 512) to (num_pts, 20 * 512)
...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_cls_2 = pt_utils.Conv2d(32, self.config.num_classes, kernel_size=(1, 1), bn=False, activation=None)
        self.dummy = pt_utils.Conv2d(32, self.config.num_dummy, kernel_size=(1, 1), bn=False, activation=None)
    ...
    def forward(self, end_points):
        ...
        dummy_out = self.dummy(features)
        features = self.fc_cls_2(features)
        f_out = features.squeeze(3)
        dummy_out = dummy_out.squeeze(3)
        end_points['logits'] = f_out
        end_points['dummy'] = dummy_out
        return end_points
...
def compute_acc_open(end_points, cfg):
    ...
    pred_unknown = logits[:, -1] >= cfg.unknown_thresh  # num_points, points predicted as unknown now true
    mask = -pred_unknown.long() + 1  # points predicted as unknown now 0

    logits = logits.max(dim=1)[1]
    logits = logits * mask + pred_unknown.long() * 15  # set point should be unknown to zero first then add 15
...
class IoUCalculator_open:
    ...
    def add_data(self, end_points):
        ...
        logits = nn.functional.softmax(logits, 1)
        # logits = nn.functional.sigmoid(logits)
        pred_unknown = logits[:, -1] >= self.cfg.unknown_thresh  # num_points, points predicted as unknown now true
        mask = -pred_unknown.long() + 1  # points predicted as unknown now 0

        logits = logits.max(dim=1)[1]
        pred = logits * mask + pred_unknown.long() * 15  # set point should be unknown to zero first then add 15
        pred_valid = pred.detach().cpu().numpy()
        labels_valid = labels.detach().cpu().numpy()
...
def compute_loss(end_points, cfg):
    ...
    dummy = end_points['dummy']

    logits = logits.transpose(1, 2).reshape(-1, cfg.num_classes)
    dummy = dummy.transpose(1, 2).reshape(-1, cfg.num_dummy)
    labels = labels.reshape(-1)
    dummy_label = torch.ones_like(labels).long() * (cfg.num_classes-1)
    ...
    valid_dummy = dummy[valid_idx, :]
    valid_dummy_label = dummy_label[valid_idx]
    ...
    #################peocess dummy###################
    # copy valid logits except for the last digit
    valid_logits[:, -1] = valid_logits[:, -1]*0
    # valid_logits_full = torch.zeros(valid_logits.size()[0], valid_logits.size()[1] + 1).to(valid_logits.device)
    # valid_logits_full[:, :-2] = valid_logits[:, :-1]
    # put valid_dummy[:,0] at the second last position at valid_logits_full representing possibility of novel class
    # valid_logits_full[:, -2] = valid_dummy[:, 0]
    # create_new dummy_logits column==label to zero and make column 16 to max of valid_dummy
    dummy_logits = valid_logits.clone()
    mask = F.one_hot(valid_labels, num_classes=cfg.num_classes)
    mask = -mask + 1
    dummy_logits = dummy_logits * mask
    valid_dummy = valid_dummy.unsqueeze(0)
    max_unknown = nn.MaxPool1d(kernel_size=4, stride=4)(valid_dummy)
    dummy_logits[:, -1] = max_unknown.squeeze()

    #################################################
    loss_normal = get_loss(valid_logits, valid_labels, cfg.class_weights)
    loss_dummy = get_loss(dummy_logits, valid_dummy_label, cfg.class_weights)

    end_points['valid_logits'], end_points['valid_labels'] = valid_logits, valid_labels
    end_points['dommy_logits'] = dummy_logits
    loss = loss_normal + 0.01 * loss_dummy
    end_points['loss'] = loss
    return loss, end_points


def compute_loss_open(end_points, cfg):
    logits = end_points['logits']
    labels = end_points['labels']
    # dummy = nn.functional.softmax(end_points['dummy'],dim=1)
    dummy = end_points['dummy']

    logits = logits.transpose(1, 2).reshape(-1, cfg.num_classes)
    dummy = dummy.transpose(1, 2).reshape(-1, cfg.num_dummy)
    labels = labels.reshape(-1)
    dummy_label = torch.ones_like(labels).long() * 15

    # Boolean mask of points that should be ignored
    ignored_bool = labels == 0
    for ign_label in cfg.ignored_label_inds[:1]:
        ignored_bool = ignored_bool | (labels == ign_label)
    # Collect logits and labels that are not ignored
    valid_idx = ignored_bool == 0
    valid_logits = logits[valid_idx, :]
    ...
    valid_labels = torch.gather(reducing_list, 0, valid_labels_init)
    #################peocess dummy###################
    # let logits position 15 to zero
    valid_logits[:, -1] = valid_logits[:, -1] * 0
    # create_new dummy_logits column==label to zero and make column 15 to max of valid_dummy
    valid_dummy = valid_dummy.unsqueeze(0)
    max_unknown = nn.MaxPool1d(kernel_size=4, stride=4)(valid_dummy) / 3
    valid_logits[:, -1] = max_unknown.squeeze()
