...
parser.add_argument('--log_dir', default='incre_3', help='Dump dir to save model checkpoint [default: log]')
...
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=FLAGS.batch_size, shuffle=False, num_workers=64, worker_init_fn=my_worker_init_fn, collate_fn=TRAIN_DATASET.collate_fn,sampler=train_sampler)
TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=FLAGS.batch_size*2, shuffle=False, num_workers=64, worker_init_fn=my_worker_init_fn, collate_fn=TEST_DATASET.collate_fn,sampler=test_sampler)
...
def load_finetune_checkpoint(net, path):
    m = torch.load(path, map_location='cuda:0')['model_state_dict']
    model_dict = net.state_dict()
    for k in m.keys():
        if k == 'fc_cls.conv.weight' or k == 'fc_cls.conv.bias':
            continue
...
if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
    ...
    # load_finetune_checkpoint(net, CHECKPOINT_PATH)
...
if torch.cuda.device_count() > 1:
    ...
    net = torch.nn.parallel.DistributedDataParallel(net, device_ids=[FLAGS.local_rank], find_unused_parameters=True)
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, FLAGS.max_epoch):
        ...
        if EPOCH_CNT % 1 == 0: # Eval every 10 epochs
